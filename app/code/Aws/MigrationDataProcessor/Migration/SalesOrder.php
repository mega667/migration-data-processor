...
namespace Aws\MigrationDataProcessor\Migration;
 ...
 */
class SalesOrder implements StageInterface
{
    /**
     ...
     * @param \Migration\Step\SalesOrder\Helper $helper
     * @param Logger $logger
     */
    public function __construct(
        ...
        \Migration\Step\SalesOrder\Helper $helper,
        Logger $logger,
        ...
        foreach ($sourceDocuments as $sourceDocName) {
            ...
            while (!empty($bulk = $this->source->getRecords($sourceDocName, $pageNumber))) {
                ...
                foreach ($bulk as $recordData) {
                    ...
                    if ($this->orderIsAllowed($destRecord->getValue('increment_id'))) {
    ...
    public function initMap($config) {
        $args = [
            $this->objectManager,
            $config,
            Map::class
        ];
        $mapFactory = new MapFactory(...array_values($args));
        $this->map = $mapFactory->create('sales_order_map_file');
    }

    protected function orderIsAllowed($incrementId)
    {
        $forbiddenIncrementIds = [
            '100000057',
            '100000708',
            '100001121',
            '100001253',
        ...
        if (in_array($incrementId, $forbiddenIncrementIds)) {
            return false;
        }
